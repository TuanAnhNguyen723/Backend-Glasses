...
@section('content')
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-[#e7edf3] dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 bg-[#f6f7f8]/50 dark:bg-slate-800/50 border-t border-[#cfdbe7] dark:border-slate-800 flex items-center justify-between">
            <p class="text-xs font-medium text-[#4c739a]" id="pagination-info">Hiển thị 0 đến 0 trong 0 đơn hàng</p>
    ...
    <script>
        (function() {
            ...
            function renderPagination(current, lastPage) {
                ...
                var html = '';
                html += '<button type="button" data-page="' + (current - 1) + '" class="order-page-btn size-8 flex items-center justify-center rounded-lg border border-[#cfdbe7] dark:border-slate-700 bg-white dark:bg-slate-900 text-[#4c739a] hover:text-primary transition-colors disabled:opacity-50" ' + (current === 1 ? 'disabled' : '') + '><span class="material-symbols-outlined text-sm">chevron_left</span></button>';
                var start = Math.max(1, current - 2);
                var end = Math.min(lastPage, current + 2);
                if (start > 1) {
                    html += '<button type="button" data-page="1" class="order-page-btn size-8 flex items-center justify-center rounded-lg border border-[#cfdbe7] dark:border-slate-700 bg-white dark:bg-slate-900 text-[#4c739a] hover:text-primary text-xs font-bold transition-colors">1</button>';
                    if (start > 2) html += '<span class="px-2 text-[#4c739a]">...</span>';
                }
                for (var i = start; i <= end; i++) {
                    var active = i === current;
                    html += '<button type="button" data-page="' + i + '" class="order-page-btn size-8 flex items-center justify-center rounded-lg border text-xs font-bold transition-colors ' + (active ? 'border-primary bg-primary text-white' : 'border-[#cfdbe7] dark:border-slate-700 bg-white dark:bg-slate-900 text-[#4c739a] hover:text-primary') + '">' + i + '</button>';
                }
                if (end < lastPage) {
                    if (end < lastPage - 1) html += '<span class="px-2 text-[#4c739a]">...</span>';
                    html += '<button type="button" data-page="' + lastPage + '" class="order-page-btn size-8 flex items-center justify-center rounded-lg border border-[#cfdbe7] dark:border-slate-700 bg-white dark:bg-slate-900 text-[#4c739a] hover:text-primary text-xs font-bold transition-colors">' + lastPage + '</button>';
                }
